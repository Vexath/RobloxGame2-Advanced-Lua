...
function Equipped()
	...
	HitConnection = Handle.Touched:Connect(function(hit)
		...
		if HitHumanoid and HitHumanoid ~= Humanoid then
			...
			if game.Players:GetPlayerFromCharacter(Tool.Parent) ~= game.Players:GetPlayerFromCharacter(Humanoid.Parent) then
				...
				game.ReplicatedStorage.Remotes.Gameplay.KnifeKill:Fire(Killer,Victim,Humanoid,"Melee");
